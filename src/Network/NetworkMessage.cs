...
public static class MessageBuilder
{
    ...
    private static NetworkMessage ParseTcpMessage(byte[] data, bool includeData = false)
    {
        ...
        if (LogItem(LogLevel.Debug)) Console.WriteLine($"{(targetId == Server.SERVER_ID ? "[SERVER]" : "[CLIENT]")} (ReadMessage) MessageType:{messageType}, SenderId:{senderId}, TargetId:{targetId}, MessageId:{messageId}: {payload}");
    ...
    internal static byte[] CreateUdpMessage(NetworkMessage msg)
    {
        ...
        if (LogItem(LogLevel.Debug)) Console.WriteLine($"{(msg.SenderId == Server.SERVER_ID ? "[SERVER]" : "[CLIENT]")} (CreateMessage) SenderId:{msg.SenderId}, TargetId:{msg.TargetId}, MessageId:{msg.MessageId}: {msg.Payload}");
    ...
    internal static NetworkMessage ReadUdpMessage(byte[] data, bool includeData = false, int? senderId = null)
    {
        ...
        if (LogItem(LogLevel.Debug)) Console.WriteLine($"{(targetId == Server.SERVER_ID ? "[SERVER]" : "[CLIENT]")} (ReadMessage) SenderId:{sender}, TargetId:{targetId}, MessageId:{messageId}: {payload}");
